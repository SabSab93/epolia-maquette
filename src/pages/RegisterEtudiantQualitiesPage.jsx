...
export function RegisterEtudiantQualitiesPage() {
  ...
  const [qualities, setQualities] = useState(data.keyQualities)
  const [query, setQuery] = useState('')

  useEffect(() => {
    setQualities(data.keyQualities)
  }, [data.keyQualities])
  ...
  const addQuality = (rawValue) => {
    ...
    if (!value) return
    if (qualities.some((quality) => quality.toLowerCase() === value.toLowerCase())) return
    if (qualities.length >= 5) return
    setQualities((current) => [...current, value])
    setQuery('')
  }
    ...
    <MobileShell>
      <div className="flex min-h-screen flex-col gap-5 p-5 pb-8">
        ...
        <section className="space-y-3">
          <label htmlFor="quality-search" className="sr-only">
          ...
          <div className="flex flex-wrap gap-2">
            {qualities.map((quality) => (
              <button
                key={quality}
                type="button"
                onClick={() => setQualities((current) => current.filter((item) => item !== quality))}
          ...
          <p className="text-xs text-epolia-muted">{qualities.length}/5 qualités ajoutées.</p>
        </section>
        ...
        <div className="mt-auto pb-2">
          <PrimaryButton
            ...
            onClick={() =>
              navigate('/register-etudiant-life', {
                state: {
                  profileForm: data.profileForm,
                  formations: data.formations,
                  portfolios: data.portfolios,
