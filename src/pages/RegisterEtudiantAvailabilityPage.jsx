...
export function RegisterEtudiantAvailabilityPage() {
  ...
  const [availabilities, setAvailabilities] = useState(data.availabilities)
  ...
  const progress = (9 / steps.length) * 100

  const addAvailability = (rawValue) => {
    const value = rawValue.trim()
    if (!value) return
    if (availabilities.some((item) => item.toLowerCase() === value.toLowerCase())) return
    setAvailabilities((current) => [...current, value])
    setQuery('')
  }
          ...
          <input
            ...
            onKeyDown={(event) => {
              if (event.key === 'Enter') {
                ...
                addAvailability(query)
              }
            }}
            placeholder="Exemple: Lundi soir"
          ...
          <div className="flex flex-wrap gap-2">
            {availabilities.map((item) => (
              <button
                key={item}
                type="button"
                onClick={() => setAvailabilities((current) => current.filter((value) => value !== item))}
